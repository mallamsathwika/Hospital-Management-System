  ...
  try {
    ...
    if (!diseaseId || !startTime || !endTime) {
      return res.status(400).json({
        message:
          "Missing required parameters: disease, startTime, and endTime are required",
      });
    }
    ...
    console.log(
      `Found ${consultations.length} consultations matching the criteria`
    );
    ...
    for (const consultation of consultations) {
      ...
      if (consultation.reports && consultation.reports.length > 0) {
        ...
        consultation.reports.forEach((report, index) => {
          // Create a metadata file for each report
          const reportMeta = {
            ...
            updatedAt: report.updatedAt
              ...
              : "N/A",
            hasFile: !!report.reportFile,
          };

          archive.append(JSON.stringify(reportMeta, null, 2), {
            name: `${reportsFolderPath}/report-${index + 1}-metadata.json`,
          });

          // Add note about accessing the actual report file
          if (report.reportFile) {
            const fileNote = `The full report file can be accessed through the HMIS system or downloaded directly from the report viewer interface.`;
            archive.append(fileNote, {
              name: `${reportsFolderPath}/report-${index + 1}-access-note.txt`,
            });
          }
        });
      } else {
        ...
        archive.append(noReportsInfo, {
          name: `${folderName}/reports/no-reports.txt`,
        });
      }
      ...
      if (consultation.prescription && consultation.prescription.length > 0) {
        ...
        for (const prescriptionId of consultation.prescription) {
          ...
          const prescriptionCsvPath = path.join(
            tempDir,
            `prescription-${prescriptionData._id}.csv`
          );
          ...
          if (prescriptionData.entries && prescriptionData.entries.length > 0) {
            ...
            const entriesWithMedName = prescriptionData.entries.map(
              (entry) => ({
                med_name:
                  medicineMap[entry.medicine_id?.toString()] || "Unknown",
                dosage: entry.dosage,
            ...
            archive.file(prescriptionCsvPath, {
              name: `${folderName}/prescription-${prescriptionData._id}.csv`,
            });
            tempFilesToDelete.push(prescriptionCsvPath);
          } else {
            // Create empty prescription CSV if no entries
            const emptyPrescriptionContent =
              "Medicine ID,Dosage,Frequency,Duration,Quantity,Dispensed Quantity\n";
            archive.append(emptyPrescriptionContent, {
              name: `${folderName}/prescription-${prescriptionData._id}.csv`,
            });
          }
          ...
          const prescriptionInfo = `
                ...
                Date: ${
                  prescriptionData.prescriptionDate
                    ? new Date(prescriptionData.prescriptionDate).toISOString()
                    : "N/A"
                }
                Status: ${prescriptionData.status || "N/A"}
            `;

          archive.append(prescriptionInfo, {
            name: `${folderName}/prescription-${prescriptionData._id}-info.txt`,
          });
        }
      } else {
        ...
        const emptyPrescriptionContent =
          "Medicine ID,Dosage,Frequency,Duration,Quantity,Dispensed Quantity\n";
        archive.append(emptyPrescriptionContent, {
          name: `${folderName}/prescription-none.csv`,
        });
      }
      ...
      if (patient && patient.vitals && patient.vitals.length > 0) {
        ...
        const filteredVitals = patient.vitals.filter((vital) => {
          ...
          return (
            vitalDate >= new Date(startTime) && vitalDate <= new Date(endTime)
          );
        });
        ...
        if (filteredVitals.length > 0) {
          ...
          const emptyVitalsContent =
            "Date,Time,Blood Pressure,Body Temperature,Pulse Rate,Breathing Rate\n";
          archive.append(emptyVitalsContent, {
            name: `${folderName}/vitals.csv`,
          });
        }
      } else {
